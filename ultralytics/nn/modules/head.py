...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "OBBWithHtp"
...
class OBBWithKpt(Detect):
    ...
    def __init__(self, nc=80, ne=1, kpt_shape=(1, 3), ch=()):
        """Initialize OBB with number of classes `nc` and layer channels `ch`."""
        super().__init__(nc, ch)
        self.ne = ne  # number of extra parameters
    ...
    def forward(self, x):
        ...
        kpt = torch.cat([self.cv5[i](x[i]).view(bs, self.nk, -1) for i in range(self.nl)], -1)
