...
class v8OBBWithHtpLoss(v8OBBWithKptLoss):
        ...
        for p in keypoints:
            ...
            for idx, pt in enumerate(p):
                if pt[2] > 0:
                    ...
                    br = int(round(x + 3 * sigma + 2)), int(round(y + 3 * sigma + 2))

                    c, d = max(0, -ul[0]), min(br[0], output_size[0]) - ul[0]
                    a, b = max(0, -ul[1]), min(br[1], output_size[1]) - ul[1]
    ...
    def single_keypoint_on_heatmap_loss(self,gt_heatmap,pred,proto,area):
        ...
        Args:
            gt_heatmap (torch.Tensor): Ground truth mask of shape (n, H, W), where n is the number of objects.
            pred (torch.Tensor): Predicted mask coefficients of shape (n, 32).
            proto (torch.Tensor): Prototype masks of shape (32, H, W).
            area (torch.Tensor): Area of each ground truth bounding box of shape (n,).
    ...
    def calculate_keypoints_on_heatmap_loss(
        ...
        Returns:
            (torch.Tensor): The calculated loss for keypoints on heatmap.
